...
public class CodableFeedStore: FeedStore {
    ...
    // we proved that 'retrieve' has no side effects so it can be run concurrently but other not
    ...
    // here we should set flags: .barrier to operating has side effects so operation are running, they will put queue on hold until they are done
